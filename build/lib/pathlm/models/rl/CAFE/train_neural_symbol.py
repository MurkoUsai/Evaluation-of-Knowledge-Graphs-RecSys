...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for split_name, dataloader in splits_to_compute:                    
            ...
            #evaluation.log(f'std_{split_name}_reward',np.std(evaluation.history( f'{split_name}_reward', iter_counter)) )
            info = ""
            ...
            if epoch % 10 == 0:
                ...
                #evaluation.push_model(policy_file, f'{MODEL}_{args.dataset}_{epoch}')
    makedirs(args.dataset)
    metrics.write(TEST_METRICS_FILE_PATH[args.dataset])#evaluation.write(os.path.join(TMP_DIR[args.dataset], VALID_METRICS_FILE_NAME))
    metrics.close_wandb()
