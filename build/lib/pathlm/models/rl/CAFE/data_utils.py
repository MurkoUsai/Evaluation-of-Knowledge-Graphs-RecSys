...
from pathlm.knowledge_graphs.kg_utils import USER, KG_RELATION
from pathlm.models.rl.CAFE.cafe_utils import load_kg
...
def test_kgmask(args):
    ...
    kgmask = KGMask(kg)
    pids = kgmask.get_mask(USER, [1, 2, 3], INTERACTION[args.dataset])
    print(pids.shape)
    pids = kgmask.get_mask(USER, 1, INTERACTION[args.dataset])
    print(pids.shape)
    pids = kgmask.get_et(USER, 1, INTERACTION[args.dataset])
    print(pids)
