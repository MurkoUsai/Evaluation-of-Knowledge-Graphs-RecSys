...
- check cartelle esistano, /data/{dataset name}/preprocessed/torchkge, altrimenti creala
"""
...
def get_watched_relation_idx(dataset):
    ...
    WATCHED=relation_df[relation_df['kb_relation'] == 'watched']['id'].astype(int).to_list()[0]
    return WATCHED
...
def dataset_preprocessing(dataset): 
    ...
    for i,uid in enumerate(users_df):
        updated_users_to_e_map.append([i+e_last_eid,f"user {uid}",uid])
    updated_users_to_e_map_df=pd.DataFrame(updated_users_to_e_map,columns=['eid','name','entity'])
    ...
    user_mapping_df.to_csv(f"{preprocessed_torchkge}user_mapping.txt",sep="\t",index=False)
    ...
    e_new_df.to_csv(f"{preprocessed_torchkge}e_map_with_users.txt",sep="\t",index=False)

    """carico train e valid, prendo solo uid e pid"""
    train_df=pd.read_csv(train_path,sep="\t",names=['uid', 'pid', 'rating', 'timestamp'])[['uid','pid']]
    valid_df=pd.read_csv(valid_path,sep="\t",names=['uid', 'pid', 'rating', 'timestamp'])[['uid','pid']]
    ...
    pid2eid={pid:eid for pid, eid in zip(i2kg_df['pid'],i2kg_df['eid'])} # pid2id [!]
    entity2eid={entity:eid for eid, entity in zip(e_new_df['eid'],e_new_df['entity'])} # mapping uid

    """creo le nuove triplette da train con la relazione watched"""
    triplets = [[uid, pid, WATCHED] for uid, pid in zip(train_df['uid'], train_df['pid'])]
    triplets_valid = [[uid, pid, WATCHED] for uid, pid in zip(valid_df['uid'], valid_df['pid'])]
    ...
    triplets_df=pd.DataFrame(triplets,columns=['entity_head','entity_tail','relation'])
    triplets_valid_df=pd.DataFrame(triplets_valid,columns=['entity_head','entity_tail','relation'])
    positives_triplets=pd.concat([triplets_df,triplets_valid_df])
    positives_triplets.to_csv(f"{preprocessed_torchkge}triplets_train_valid.txt",sep="\t",index=False)

    """carico il kg e gli concateno le nuove triplette (triplets), quindi salvo le nuove triplette"""
    kg_df=pd.read_csv(kg_new_final,sep="\t")[['entity_head','entity_tail','relation']]
    
    triplets_df.to_csv(f"{preprocessed_torchkge}triplets_created.txt",sep="\t", index=False)

    kg_updated=pd.concat([kg_df,triplets_df])
    kg_updated.to_csv(f"{preprocessed_torchkge}kg_final_updated.txt",sep="\t",index=False)
    ...
    test_df = pd.read_csv(test_path, sep="\t", names=['uid', 'pid', 'rating', 'timestamp'])
    triplets_test = [[uid, pid, WATCHED] for uid, pid in zip(test_df['uid'], test_df['pid'])]
    ...
    triplets_test.to_csv(f"{preprocessed_torchkge}triplets_test.txt",sep="\t",index=False)
